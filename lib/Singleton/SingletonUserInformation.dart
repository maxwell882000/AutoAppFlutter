...
class SingletonUserInformation {
  ...
  void setNewCard(int id) {
    _newCard =
        _cards.card.firstWhere((element) => element.id == id, orElse: null);
  }
  ...
  List createList(CardUser element) {
    ...
    if (element.change.run == 0) {
      ...
      return [
        ...
        percentage,
        current,
        _average == 0 ? "незивестно" : translatedDistance,
        element.id,
        element.date
      ];
    } else {
      ...
      return [
        element.nameOfCard,
        percantage,
        _average == 0 ? "неизвестно" : days.toDouble(),
        rest,
        element.id,
        element.date
      ];
  ...
  void fromJson(Map<String, dynamic> json) {
    ...
    if (json['cards_user'] != null) cards.fromJson(json['cards_user']);
    else{
      cards.clean();
    }
  }
  ...
  void updateRun() {
    http.put("https://autoapp.elite-house.uz/transport/$id/",
...
class Cards {
  ...
  Future<void> loadCards() async {
    ...
    if (_id != null && _id != 0) {
      ...
      final result = await http.get("http://autoapp.elite-house.uz/cards/$id/");
      List json = jsonDecode(utf8.decode(result.bodyBytes));
      if (json.isNotEmpty)
        json.forEach((e) => _card.add(new CardUser(
            e['id'],
            e['name_of_card'],
